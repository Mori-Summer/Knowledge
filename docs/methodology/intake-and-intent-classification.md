...
related_docs:
  ...
  - docs/governance/revision-regeneration-continuity-policy.md
open_questions:
  - Epic 2 frontmatter schema 落地后，是否需要把部分 intake 输出字段 schema 化？
  - 如果 Epic 3 review/completion templates 后续调整 scope classification 字段，是否需要把 intake decision record 抽成固定报告项？
...
本文不替代主方法论、概念文档生成合同、模板、质量门禁、固定 Prompt、生命周期政策、版本治理政策、导航政策、已落地 Epic 3 review/completion assets 或批量 readiness 政策；也不提前定义 Epic 2 的 frontmatter/index schema 或 Epic 6 的 batch runbook。
...
- `defer_to_later_story`：缺口属于明确未来 story，例如 Epic 2 schema、Epic 5 link taxonomy 或 Epic 6 runbook。
...
- Epic 2 frontmatter schema、topic/path/naming、candidate promotion 和 index synchronization rules 落地后，复核本文是否需要更正式的 intake decision record。
- Epic 3 review record、document decision policy、rework loop 或 completion report template 发生实质字段或 vocabulary 变更时，复核本文的验证证据是否应同步到固定模板。
...
- Epic 5 related docs taxonomy 和 link maintenance policy 落地后，复核 related docs、successor 和 owner entry 的边界。
