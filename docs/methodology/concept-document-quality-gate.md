...
它不要求每篇文档一样长，但要求每篇文档都能稳定回答所属路径规定的核心问题。
...
- 没有迁移与关联模型
- 没有未解问题与继续深挖
- 对模型型主题，没有核心机制 / 主链路 / 因果链
- 对模型型主题，没有关键 tradeoff 与失败模式
- 对纯概念主题，没有什么算它 / 什么不算它、例子 / 反例或常见误读
- 对明显复杂对象，各核心章节只用极短摘要草草带过，无法支撑判断、选型或迁移

### 3.3 模型 / 判别质量 Hard Fail

- 文档主要是在复述定义，而不是建立可复用内部模型，也不是建立稳定概念辨识力
- 模型型文档没有说明“为什么这样”，只有“它是什么”
- 模型型文档没有把机制压成可调用判断框架
- 纯概念文档没有建立清晰的分类边界、误读纠偏或相邻概念区分
- 文档没有指出边界、适用条件、失效条件或误用边界
...
- 模型型主题是否有主链路 / 因果链
- 纯概念主题是否有足够稳定的判别框架
- 结构是否能支持后续解释、预测、区分和判断
- 展开密度是否与主题复杂度匹配；对明显复杂对象，是否至少达到 `deep` 模式应有的展开度

### 4.3 Tradeoff / 失败模式 / 误读纠偏

检查：

- 模型型主题是否指出核心取舍
- 模型型主题是否说明失效条件
- 纯概念主题是否指出常见误读、错误类比或过度泛化
- 失败模式或误读纠偏是否具体，而不是空泛提醒
...
1. 先判断文档属于模型型概念文档还是纯概念文档。
2. 再判断主题复杂度，以及它应按 `standard` 还是 `deep` 审视。
3. 再查仓库集成与 frontmatter。
4. 再查结构完整性。
5. 再看正文是否真的提供了内部模型或概念辨识框架。
6. 最后再看工业锚点、当前实践、来源和时间敏感性。
