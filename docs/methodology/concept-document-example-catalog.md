...
related_docs:
  ...
  - docs/governance/prompt-template-quality-version-governance.md
open_questions:
  - Epic 3 建立 review record、decision policy 和 rework loop 后，是否需要把本文的 reviewer action calibration 替换为正式决策链接？
  - Epic 2 建立 frontmatter schema 后，是否需要调整本目录中的示例 frontmatter 表达方式？
  - 后续如果来源纪律政策新增 schema 化标签，是否需要复核本文示例中的标签展示方式？
---
...
本文不替代主方法论、概念文档生成合同、intake 流程、模板、质量门禁、固定 Prompt、生命周期政策、版本治理政策、导航政策、source-discipline policy，或未来 Epic 2、Epic 3、Epic 6 的 schema、review record、decision policy、rework loop、completion report、batch runbook。
...
这些动作是样例校准启发，不是正式 document decision policy。正式 accept、revise、regenerate、reject、hold 规则、review record 字段、rework-loop examples 和 completion-report templates 由 Epic 3 负责，本文不提前创建。
...
related_docs:
  ...
  - docs/methodology/concept-document-quality-gate.md
open_questions:
  - 来源纪律政策落地后，是否需要在未来示例集中补一个外部来源纪律案例？
---
...
- Epic 2 的 schema 规则落地后，边界证据是否需要独立字段表达？
...
| Open questions | 明确 Epic 2 和来源纪律示例后续复核点 | 避免把未决治理问题伪装成已解决 | `acceptance_candidate` |
| Source/currentness labels | 区分 `project rule`、`verified fact`、`inference`、`open question` | 满足来源与时间语境最低标签要求 | `acceptance_candidate` |

这个 passing example 不因为文字完整就直接声明 `reviewed` 或 `validated`。它保持 `quality_status: draft`，因为它是示例化校准资产中的片段，且来源纪律政策、Epic 2、Epic 3 仍可能细化示例展示、schema 和决策规则。
...
Reviewer 可以用下面启发判断候选稿该补写、重生成还是接受。它们只服务 Story 1.3 样例校准，不是 Epic 3 的最终决策政策。
...
实际完成审查时，reviewer 还必须按质量门禁输出 Hard Fail、六项评分或等价治理检查、必改项、可改进项、最终结论和允许状态。Epic 3 会定义正式 review record、decision policy、rework loop 和 completion report。
...
| `open question` | 当前 story 不解决、尚待后续核查或归属未来 Epic 的事项 | “Epic 2 后是否调整示例 frontmatter 表达？” |
...
| 验证项 | 通过证据 |
| --- | --- |
| role/authority/scope | 本文声明自己是 reviewer calibration support asset，且不替代主规范、合同、模板、质量门禁、fixed prompt、source policy 或未来 Epic 资产 |
...
| reviewer actions | 只定义 `targeted_revision`、`regeneration_candidate`、`acceptance_candidate` 的校准启发，不提前创建 Epic 3 policy |
