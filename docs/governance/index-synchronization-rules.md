...
time_context: phase_4_epic_2_index_synchronization_2026_05_26
...
- Future templates/runbooks 只有在目录和入口被明确批准后，才建立对应 section 或受控引用路径。
...
| methodology asset | `## methodology` | `./methodology/{slug}.md` | 方法论、模板、质量门禁、fixed prompt、playbook 或方法论支撑资产 |
...
| future template asset | future `## templates` 或受控 entry point | `./templates/{slug}.md` | 只有后续 story 或 Maxwell 明确批准后使用 |
...
| future template/runbook | 未默认开放 | 等后续 story 或 Maxwell 批准目录和入口后再索引 |
...
7. Rename、move、retitle 或 topic change 范围内没有 stale old path/title entry。
8. 没有把 `_bmad-output/`、conversation output、temporary draft、story file、review output 或 completion record 加入正式索引。
9. 没有把 `docs/_reports/` 重分类为普通 topic；正式 reports 保留 controlled special directory 语义。
10. 没有未经授权新增 schema fields、index schema、generated index system、lint/scoring tool、software tests、package manifest、`src/`、build config、runtime automation、CLI/API/UI/database/deployment/CI artifact。
11. 如果没有修改索引，completion evidence 说明了 `not_applicable`、`referenced_elsewhere`、`intentionally_excluded`、`deferred_with_reason` 或 blocking state。
12. 如果触及多个文件、多个 topics、多个 index entries 或由规则选出目标集，已先满足 `docs/governance/batch-readiness-checklist.md`。
