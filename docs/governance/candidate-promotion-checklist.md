...
related_docs:
  ...
  - docs/methodology/governance-asset-boundary-policy.md
open_questions:
  - Story 4.1 已建立 Revision / Regeneration Continuity Record；是否需要在后续专门审查中把 promotion replacement evidence 与该 record 进一步对齐？
  - Story 2.6 已定义 duplicate/coexistence policy 后，是否还需要为 same-topic candidate coexistence 增加更细判定表？
...
本文是 `Knowledge` 项目中候选文档晋升为正式 `docs/` 资产的 canonical governance asset。它回答的是：对话输出、`_bmad-output/` 草稿、planning output、review output、completion record、临时 Markdown 或手工候选稿，进入正式知识库前必须检查什么、记录什么、什么时候停止，以及哪些判断必须留给后续 story 或 Maxwell 确认。
...
本文补充 `docs/methodology/document-generation-methodology.md`、`docs/methodology/concept-document-contract.md`、`docs/methodology/intake-and-intent-classification.md`、`docs/methodology/concept-document-quality-gate.md`、`docs/methodology/source-discipline-and-real-world-anchor-policy.md`、`docs/governance/frontmatter-schema.md`、`docs/governance/topic-path-naming-policy.md`、`docs/governance/governance-asset-navigation-policy.md`、`docs/governance/lifecycle-states.md` 和 `docs/governance/batch-readiness-checklist.md`。

本文不替代主方法论、概念文档合同、质量门禁、Story 2.1 frontmatter schema 或 Story 2.2 topic/path/name policy。晋升时必须调用这些资产，而不是在本文中重建第二套 schema、路径政策或质量门禁。
...
当前 `quality_status: draft` 是保守治理状态。原因是本文是 Epic 2 首版 promotion checklist；Story 2.4 已细化 index synchronization，Story 2.5 已细化 rename/migration，Story 2.6 已细化 duplicate/coexistence，后续 Epic 3、Epic 5 和 Epic 6 仍会细化 review records、related docs 和 batch runbooks。
...
| `defer_with_reason` | 需要 Story 2.5、2.6、Epic 3、Epic 5 或 Epic 6 的专门规则，并且本轮必须记录 defer reason、impact、owner/future story 和 re-review trigger |
| `replace_with_continuity_record` | 替换已有 formal asset，且身份连续、旧内容处理和引用影响已记录 |
| `not_authorized` | 候选需要的文件范围、状态声明、自动化、批量操作或 future-story 资产未被当前 story 授权 |
...
- Future template 或 future runbook：只有在后续 story 或 Maxwell 明确批准后进入对应目录。
...
项目内治理规则可以由 formal docs、当前 story、BMad workflow 或 Maxwell 明确指令支撑；它们不能被写成外部行业事实。
...
- missing/planned targets 被记录为 open questions 或 future-story dependency。
...
| index impact | 使用条件 |
| --- | --- |
| `updated` | 新增、移动、重命名、改标题、改 topic 或改变导航入口，且当前 story 授权更新 |
...
| `deferred_with_reason` | 发现需要同步，但详细规则、目标集或 owner decision 属于后续 story |
...
`blocked_pending_story_2_4` 是 Story 2.4 落地前的历史占位状态。Story 2.4 之后的新记录不得继续使用该值；未解决 index impact 应改为 `deferred_with_reason` 并命名后续 owner，或在存在索引政策冲突时使用 `blocked_index_policy_conflict` 并停止处理。

### 10. Operator decision

最后记录 operator decision。没有 decision record 的候选不能算完成晋升。Decision record 可以写在正文、review evidence、completion evidence、story Dev Agent Record、Epic 3 review/completion templates 或 future runbook records 中；即使 Epic 3 模板已落地，也不得新增未经授权的全局 frontmatter 字段。
...
| split | 一个旧资产是否拆成多个资产？ | defer to Story 2.5，除非已有 Maxwell 明确授权 |
| merge | 多个旧资产是否合并成一个资产？ | defer to Story 2.5，除非已有 Maxwell 明确授权 |
| successor | 新资产是否成为旧资产继任入口？ | defer to Story 2.5，记录 successor ambiguity |
| deprecation/archive | 旧资产是否要降级、废弃或归档？ | defer to Story 2.5 或生命周期专门流程 |
| duplicate/coexistence | 是否存在重复概念或同主题共存风险？ | defer to Story 2.6，除非差异和共存理由已授权 |
...
- stable identity、canonical entry point、lifecycle state、broad navigation 或 existing high-value content 会受影响，且没有当前 story 或 Maxwell 明确授权。

必须请求 Maxwell 确认的情况包括：

- 替换会改变 stable `doc_id` 或 canonical entry point。
- 替换会把一个 active/maintained/validated 资产降级、废弃或归档。
- 替换会移除高价值旧内容，且不能从当前 story 或质量门禁明确推出。
...
| index impact | `updated`、`not_applicable`、`referenced_elsewhere`、`intentionally_excluded`、`deferred_with_reason` 或 `blocked_index_policy_conflict`；旧记录中的 `blocked_pending_story_2_4` 仅表示 Story 2.4 前的历史占位 |
...
| follow-up work | 后续修订、核查、review、migration 或 story dependency |
...
这些 record 字段属于正文、review evidence、completion evidence、story Dev Agent Record、Epic 3 review/completion templates 或 future runbook records。它们不是新的全局 frontmatter 字段。Story 2.3 可以定义 required evidence fields，但不创建 Epic 3 review record template、document decision policy、rework loop example 或 completion report template。
...
- promotion 需要创建或实现 Story 2.4、Story 2.5、Story 2.6、Epic 3 或 Epic 6 的未来资产。

后续归属边界：

- Story 2.4 负责详细 `docs/index.md` synchronization rules。
- Story 2.5 负责 rename、migration、split、merge、deprecation、successor 和 link-impact policy。
- Story 2.6 负责 duplicate concept 和 same-topic coexistence policy。
- Epic 3 负责 review record template、document decision policy、rework loop examples 和 completion report template。
...
未来 Agent 或 reviewer 在 candidate promotion 前，至少按下面清单逐项确认：
...
4. 目标 asset class、topic、path、filename、`concept` 和 `doc_id` 已按 Story 2.1 与 Story 2.2 检查。
...
11. `docs/index.md` impact 已分类为 `updated`、`not_applicable`、`referenced_elsewhere`、`intentionally_excluded`、`deferred_with_reason` 或 `blocked_index_policy_conflict`；旧记录中的 `blocked_pending_story_2_4` 已按 Story 2.4 视为历史占位。
...
15. 未创建 runtime code、package manifest、source tree、software tests、build config、automation、CLI/API/UI/database/deployment/CI 或 future story assets。
...
- Story 2.4 已建立 detailed `docs/index.md` synchronization rules；后续若该政策更新，复核本文的 index impact vocabulary。
- Story 2.5 建立 rename、migration、split、merge、deprecation、successor 和 link-impact policy。
- Story 2.6 已建立 duplicate concept 与 same-topic coexistence policy；后续若该政策更新，复核本文的重复/共存 disposition。
- Epic 3 review record template、document decision policy、rework loop examples 或 completion report template 发生实质字段或 vocabulary 变更。
- Story 4.1 已建立 revision/regeneration continuity policy；后续若该政策更新，复核 promotion replacement、old-content handling 和 continuity evidence。
- Story 4.2 或 Story 4.3 建立 sidecar boundary 或 legacy migration guide 后，复核 candidate promotion 与 sidecar/legacy boundaries。
