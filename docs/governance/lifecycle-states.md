...
related_docs:
  ...
  - docs/governance/network-boundary-and-decay-prevention.md
open_questions:
  - 质量状态基线、文档决策政策或完成汇报模板后续更新时，是否需要调整本资产中的 quality_status 兼容映射？
  - frontmatter schema 已建立后，是否需要新增独立 lifecycle_state 字段？
  - index synchronization rules 已建立后，是否需要为 deprecated 与 archived 资产建立单独索引分组？
---
...
当前 `quality_status: reviewed` 表示本文已完成 Epic 6 前置稳定化审查：生命周期词表、状态转换证据、quality_status 兼容映射、owner/index entry、相关治理依赖、链接/索引边界和非软件边界已检查。未解决项保留在 `open_questions` 和维护触发点中；本文不声明 `validated`，因为 Epic 6 batch governance runbook、batch review record 和 batch completion report 仍未落地，独立 `lifecycle_state` frontmatter 字段也未被授权。
...
`quality_status` 表达质量结果、维护状态或与既有质量门禁相关的状态信号。它可以包含 `draft`、`maintained_asset`、`upgraded_v1` 等既有值。Story 0.5 已固化质量状态基线；本资产只建立兼容解释，不静默重写历史字段。
...
Frontmatter schema 尚未授权正式 `lifecycle_state` frontmatter 字段。本文不向新资产或现有资产添加该字段。生命周期可在正文、审查记录、完成报告、废弃说明或后续 schema 中表达；是否加入 frontmatter 仍是 open question。
...
- Prompt、template 与 quality rule version governance 更新后，复核 `prompt_version`、`template_version` 与 lifecycle transition 的关系。
- 质量门禁 Hard Fail、评分维度与质量状态基线更新后，复核 `quality_status` 兼容映射和 `validated` 证据要求。
- `docs/governance/governance-asset-navigation-policy.md` 已建立导航入口归属基线；`docs/governance/index-synchronization-rules.md` 已定义详细 index synchronization rules，后续更新时复核 `deprecated`、`archived` 和 index treatment 规则。
- `docs/governance/batch-readiness-checklist.md` 已定义批量治理 readiness；批量迁移、批量废弃和批量归档必须先按该政策完成 target set、冲突暴露、停止条件和恢复策略判定。
- Frontmatter schema、`doc_id`、topic/path/name policy 和 candidate promotion rules 更新后，决定是否新增正式 `lifecycle_state` 字段。
- Review record、document decision policy、rework loop 或 completion report template 发生实质字段或 vocabulary 变更时，复核 lifecycle evidence 的报告格式。
