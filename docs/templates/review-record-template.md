...
open_questions:
  ...
  - Story 6.2 建立 batch review record 后，是否需要扩展本文的 batch-readiness / batch-runbook evidence 部分？
---
...
- Story 6.2 batch review record 或 Story 6.3 batch completion report。
...
| Batch Readiness Record | 目标集由规则选出、影响多个资产/topics/status/index entries，或需要批量治理时 | 由 batch readiness checklist 保持字段权威；批量执行约束由 Story 6.1 runbook 补充；未来 Story 6.2/6.3 资产负责专门 batch review/completion evidence |
...
不得把 review/decision metadata 作为 unauthorized frontmatter fields 写入目标资产。需要持久化的 review evidence 应留在 review record、completion evidence、目标正文的合规说明、story Dev Agent Record、Epic 3 review/completion evidence、Story 6.1 runbook 允许的执行证据位置或未来 Story 6.2/6.3 专门 records 中。
...
- Story 6.2 batch review record 或 Story 6.3 batch completion report 建立后。
