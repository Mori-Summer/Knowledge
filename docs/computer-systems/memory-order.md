...
source_basis:
  ...
  - atomicity_modification_order_release_sequence_and_fence_docs_merged_2026_06_18
  - docs_folder_consolidation_progress_2026_06_23
time_context: current_practice_checked_2026_03_16_and_memory_model_docs_consolidated_2026_06_23
...
- atomicity 管单对象的一次访问是否不可分割。
- memory order 管这次原子访问周围的普通读写，能不能跟着建立顺序、发布和获取关系。

最短边界句是：**原子先回答“会不会被撕裂”，order 再回答“别的效果能不能被带过去”。**

**`modification order` vs 全局时间线**

每个 atomic object 各有一条自己的修改总序。`x` 的账本不会自动和 `y` 的账本拼成一条全程序时间线。load 会参考这份账本，但不会作为新修改点进入账本；失败 CAS 也不会新增修改点。

**`release sequence` vs 同一状态概念**

release sequence 是同一 atomic object 的 modification order 上，从 release 头部出发、沿连续成功 RMW 延续的一段特殊链。普通 store、失败 CAS 或换到另一个 atomic object，都会让这条链不能按原有方式继续。

**`fence` vs 操作自带 order**

给原子操作直接写 order，是把顺序合同绑在该操作上；`atomic_thread_fence` 是把顺序切面放到一个程序点，再借 carrier atomic 和读值关系把两侧接起来。单独一条 fence 不会凭空同步全局状态。
...
### 4.3.1 单对象访问、单对象账本和跨线程同步要分层看

| 层 | 它解决什么 | 常见误读 |
| --- | --- | --- |
| Atomicity | 这个对象的一次访问是否不可分割 | 原子了就等于同步了 |
| Modification order | 这个 atomic object 自己的写入历史 | 每个原子对象的顺序能合成全局时间线 |
| Release sequence | 同一对象上 release 头部后的连续成功 RMW 链 | 同一状态概念上的后继写都能续链 |
| Memory order | 原子操作如何建立顺序、发布和获取 | 有 atomic 就默认带上周围数据 |
| Fence | 把顺序切面从具体操作拆到程序点 | 插一条 fence 就自动同步所有线程 |

这张表是本文合并后的核心压缩模型：不要把单对象访问单位、单对象历史、特殊中继链和最终同步关系混成一个词。
...
### 5.1.1 release sequence 如何让发布穿过 RMW 链

如果一个 release store 之后，同一 atomic object 上继续发生连续成功 RMW，后续 acquire 只要读到这条链上的某个值，就可能同步回头部 release。关键条件是：

- 链头必须是 release。
- 链身必须仍贴着同一 atomic object 的 modification order。
- 普通 store 不是自动续链者。
- acquire 必须读到链上的值，不能只靠关键字自证。

这也是为什么 lock-free 状态字、引用计数和阶段推进代码里，普通 store 与失败 CAS 都是高风险断链点。
...
### 5.5.1 fence 能建边，但通常需要 carrier atomic

fence 的三个常见建边模式可以压成一张表：

| 模式 | 直觉 | 必须回查什么 |
| --- | --- | --- |
| release fence -> acquire operation | 发布侧把顺序放在 fence，读取侧直接 acquire | acquire 是否读到了发布侧相关 carrier 值 |
| release operation -> acquire fence | 发布侧用 release 操作，读取侧读值后再立 acquire fence | 读线程是否先读到对应发布值 |
| release fence -> acquire fence | 两边都用 fence，中间靠原子操作接起来 | carrier atomic 和 read-from 关系是否真的成立 |

如果没有 carrier atomic object 和被观察到的值，fence 通常只能提供本线程顺序约束，不能凭空制造 `synchronizes-with`。
...
**失败模式 7：把多对象 modification order 偷偷拼成全局顺序**

每个 atomic object 各有自己的账本，跨对象关系要靠更高层边建立。

**失败模式 8：普通 store 断掉 release sequence 却没意识到**

状态字仍是同一个“概念”，不代表 release sequence 仍在同一条连续 RMW 链上。

**失败模式 9：只写 fence，不画 carrier atomic**

这类代码看起来很强，实际可能没有建立任何跨线程同步边。
...
| 后续状态推进读不到头部发布的数据 | release sequence 是否被普通 store 断掉 | RMW 链断裂 |
| 代码里有 fence 但仍读到旧 payload | carrier atomic 与 read-from 关系 | fence 没有真正建边 |
