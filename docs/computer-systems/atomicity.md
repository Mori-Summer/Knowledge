...
现实里很多标准库原子接口和底层并发原语，最终都要落回这类编译器/IR 级原子语义。

### 8.2 `std::atomic` / `atomic_ref`

当前草案把 `std::atomic` 和 `atomic_ref` 分别提供为“直接定义原子对象”和“在现有对象上施加原子访问规则”的两条正式路径。  
...
这类场景之所以反复出现，恰好说明 atomicity 的真实落点通常是“先把单对象访问变成稳定单位”，而不是直接解决整个并发协议。
