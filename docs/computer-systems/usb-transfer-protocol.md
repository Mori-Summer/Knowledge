...
source_basis:
  ...
  - usb_host_controller_path_model_merged_2026_06_18
  - docs_folder_consolidation_progress_2026_06_23
time_context: standards_and_engineering_practice_baseline_2026_03_23_and_usb_host_path_consolidated_2026_06_23
...
- 你能把一次上层请求拆成 request、HCD、descriptor/ring、DMA、bus transaction、event/completion 这条主机控制器数据路径
...
### 4.6 主机控制器数据路径：七个实现平面

USB 主机控制器路径模型已并入本文。把 USB 请求落到硬件路径时，需要在前面的协议五平面之外，再看七个实现平面：
...
| HCD 翻译平面 | 把软件请求翻成控制器工作项 | URB/IRP to TRB/TD、queue selection、ring advance |
...
| 总线事务平面 | 在线上真正发生什么 | token/data/handshake、ACK/NAK/STALL、frame/microframe |
| 完成回写平面 | 把结果重新送回 CPU | event ring、interrupt/polling、request completion、unmap/recycle |

这七个平面解释了为什么一次 API 调用、一次控制器工作项、一次 DMA 搬运和一次线上事务通常不是一一对应。
...
### 5.1.1 主机控制器桥接链：request -> ring -> DMA -> bus -> event

从主机控制器数据路径看，一次请求还要穿过这条桥：

1. 上层驱动形成请求，例如读取 bulk IN 端点、提交一批 isoch buffer，或发一个 control request。
2. USB core 绑定设备、接口、端点和当前配置状态。
3. HCD 把请求翻译成控制器可理解的 descriptor / TRB / TD 链。
4. payload buffer 和必要控制结构被 DMA 映射成控制器可达地址。
5. 软件推进 ring 或 queue，并用 doorbell / producer index 通知控制器。
6. 控制器 DMA 拉取描述符和 payload，并按 frame / microframe 调度真实 USB 事务。
7. 控制器把完成状态写回 event / completion。
8. 软件消费 completion，执行必要同步、unmap、回调和资源回收。

这条链的统一判断是：

**USB 主机侧的真实性能和真实故障，通常发生在“请求语义”和“硬件数据路径”之间的翻译缝隙里。**
...
6. **只看抓包，不看 ring/event/completion**
   抓包上有事务，不代表上层请求一定完成；它可能只是其中一部分事务动了。

7. **提交成功误当控制器已执行**
   驱动打印“提交成功”可能只是成功入软件队列，不代表控制器已看到 descriptor，也不代表线上事务已经发生。

8. **DMA 地址正确就误以为协议层也正确**
   DMA mapping 只解决控制器能访问 buffer；端点状态、transfer type、短包、STALL、NAK 和 completion 聚合仍然可能出问题。
...
### 8.5 Linux usbcore / HCD 与 Windows USB host stack

Linux 的 usbcore + HCD 分层和 Windows USB host stack 都说明同一件事：上层驱动提交的是请求对象，不是线上的 packet。请求表达、控制器翻译、DMA 可见性和 completion 投影这几层不会因为平台不同而消失。
...
- 调试时同时看五层：上层请求、HCD 翻译、控制器队列、DMA 路径、线上事务和 event/completion，不要只看某一层日志
...
- “一次 API 调用就等于一次线上事务”
- “抓包上有事务，就说明软件请求一定完成”
- “USB 问题只要抓包，不需要看 ring/event/completion”
...
7. request、descriptor/TRB、DMA buffer、event/completion 分别处在哪一层？
8. 如果抓包上有事务但上层一直没 completion，你该优先怀疑哪几层？
