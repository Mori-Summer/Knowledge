...
- `synchronizes-with`：具体跨线程同步边是否成立
- `data race`：冲突访问是否越过语言模型的合法边界

单对象原子访问、modification order、release sequence 和 fence 的细节，统一收束到 [Memory Order：从原子性到可见性与重排序控制](memory-order.md)。
...
### 4.4 `synchronizes-with` 是具体边，不是“差不多同步了”

`synchronizes-with` 首先是一条可检查的跨线程边。它不是口头上的“同步完成”，也不是“两个线程碰了同一个对象”。

常见成立来源可以压成这张表：

| 来源 | 成立条件 | 它接入 HB 图的方式 |
| --- | --- | --- |
| release/acquire | acquire 读取到 release 发布的值，或读取到相关 release sequence 上的值 | release 前的线程内历史传到 acquire 后 |
| mutex | 前一次 `unlock` 同一把 mutex，后续成功 `lock` 获得所有权 | 上一个临界区的写传到下一个临界区 |
| condition variable | 共享 predicate 在同一锁纪律下修改和检查，wait 醒来后重新拿锁 | notify 只是推进等待，真正状态仍靠 mutex/predicate 链闭合 |
| `atomic_wait` | 等待围绕同一个 atomic value 的修改历史和通知动作展开 | 值变化、等待资格和 notify 共同决定 waiter 是否能前进 |
| fence | 两侧 fence 或操作必须通过 carrier atomic 与 read-from 关系接起来 | 单独 fence 只约束本线程，不凭空制造跨线程边 |

不成立的典型情况也要显式记住：

- relaxed 访问同一个 atomic，不自动建立同步边。
- acquire 没读到对应 release 发布的值，不能只靠关键字自证。
- notify、wake、signal 不等于共享状态已经被对方可见。
- fence 没有 carrier atomic 和读值关系时，不会把所有线程同步起来。

### 4.5 Data race 是 HB 图的合法性出口

data race 不是“结果不稳定”的泛称，而是语言模型里的硬边界。最短判定式是：

| 条件 | 判断问题 |
| --- | --- |
| 潜在并发 | 两个访问是否可能不按同一线程内顺序固定排列？ |
| 冲突访问 | 是否访问同一内存位置，且至少一方是写？ |
| 非原子参与 | 是否至少有一个访问不是 atomic 或受等价同步保护？ |
| 缺少 HB | 两个冲突访问之间是否没有 `happens-before` 把它们隔开？ |

四项同时成立时，就不是“业务 race condition”那么简单，而是已经落入 data race。
反过来，race condition、死锁、活锁、锁竞争和缓存争用都可能很严重，但它们不自动等于 data race。
...
### 5.5 经典链路 4：atomic wait 也要回到值历史

`atomic_wait` 最容易被误解成“等某个未来事件”。更稳的模型是：

1. waiter 先观察到 atomic object 的某个 old 值。
2. 如果值仍等于 old，waiter 可以阻塞。
3. 另一个线程修改同一个 atomic object，并在合适时机 notify。
4. waiter 醒来后仍要重新检查值，因为可能 spurious wake，也可能遇到 ABA。

这里的关键不是 notify 这个动作本身，而是：

**等待协议是否围绕同一个 atomic object 的 modification order 和新的可观察值闭合。**

### 5.6 经典链路 5：图断了之后会出现两类后果
...
**失败模式 6：把 race condition 直接叫成 data race**

逻辑结果受调度影响，不代表一定有语言层 data race；必须回到潜在并发、冲突访问、非原子参与和 HB 缺失四项判定。

**失败模式 7：把“没有 sanitizer 报警”当作没有问题**

工具是在近似检查 HB 图，不是标准语义本身；工具没覆盖到不等于边已经成立。
...
- 单对象访问、单对象修改账本、release sequence、fence 和 order 选型，回到 [memory-order](memory-order.md)
- mutex、condition variable、semaphore、`atomic_wait` 的原语选型，回到 [multithreaded-locks](multithreaded-locks.md)
- data race 判定和同步边成立性，留在本文这张 HB 总图里处理
...
6. data race 和 race condition 的边界是什么？
7. 为什么 notify、fence 或 relaxed atomic 不能单独证明跨线程同步边成立？
...
- [Memory Order：从原子性到可见性与重排序控制](memory-order.md)
