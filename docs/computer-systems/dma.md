...
source_basis:
  ...
  - dma_program_memory_projection_model_merged_2026_06_18
  - dma_lifetime_state_machine_merged_2026_06_18
  - docs_folder_consolidation_progress_2026_06_23
time_context: standards_and_engineering_practice_baseline_2026_03_23_and_dma_docs_consolidated_2026_06_23
...
- [Cache Coherence：同一份数据为什么不会在不同核心缓存里“各说各话”](cache-coherence.md)
...
### 4.5 程序内存从 DMA 看是五层投影

旧的程序内存 DMA 视角文档已并入本文。把“程序缓冲区能不能 DMA”建模得足够稳，至少要拆成五层：

| 层 | 这层如何命名同一批字节 | 最容易错在哪 |
| --- | --- | --- |
| 程序语义层 | 对象、字段、数组、消息、帧 | 以为对象边界在下面几层也天然存在 |
| 进程虚拟层 | 用户 VA、VMA、页内偏移 | 把虚拟地址误当设备地址 |
| 物理页层 | PFN、物理页集合、驻留状态 | 误以为页后备天然稳定不会迁移或回收 |
| 设备可达层 | IOVA、bus address、SG 段 | 忽略映射窗口和地址空间隔离 |
| 传输执行层 | 描述符、ring index、burst | 误把“描述符提交”当成“数据已经安全可见” |

这五层还要叠加两条硬约束：

- **时间约束。** 同一组字节在 `t0` 可被程序访问，不代表在 `t1` 仍可被设备稳定访问。
- **可见性约束。** 同一组字节即便落在同一批页上，也不代表 CPU 和设备此刻看到的是同一个版本。

### 4.6 DMA 缓冲区不是两态对象，而是八态状态机

DMA 生命周期状态机已经并入本文。把缓冲区只分成“提交前/提交后”不够，最少应看到这些状态：
...
很多 DMA bug 的本体，就是把虚拟区间生命周期、物理后备生命周期、设备映射生命周期和语义所有权生命周期误看成一条线。
...
7. 这块缓冲区处于八态状态机里的哪一态，谁拥有读写权？
8. 如果原始缓冲区不适合直接 DMA，是否应该主动使用 bounce / staging / copy fallback？
...
一次程序缓冲区进入 DMA，并不是“虚拟地址翻译成物理地址”这么短，而是一串组合动作：

1. 程序先形成高层对象，例如网络报文、磁盘 I/O 缓冲区、视频帧或用户提交给驱动的切片。
2. 对象落在进程虚拟地址空间中，CPU 能访问，但设备还看不见这串地址。
3. 内核确认这段范围当前由哪些页承载，必要时 pin、锁定、换成内核分配的 DMA-safe 缓冲区，或走 bounce/staging。
4. 驱动把这些页变成设备能理解的段，可能直接形成 bus address，也可能经 IOMMU 生成 IOVA。
5. 驱动把段信息写入描述符或 ring。
6. CPU 做必要的 cache sync 与 barrier，再用 doorbell 或等价机制移交给设备。

反方向也同样需要闭环：设备写回完成后，CPU 需要消费 completion，并在必要时 invalidate / sync，再把页上的新字节重新解释成程序对象。

### 5.4 为什么 cache coherency 不等于 DMA 正确
...
8. **只懂地址映射，不懂时间窗**
   地址数值没变，不代表 DMA 依赖的页后备、映射或缓存语义在整个时间窗里都没变。

9. **一完成就立刻复用**
   completion 不是 sync，也不是自动安全复用；中间还可能差 CPU 侧同步、unmap 和语义消费。

10. **长期 pin 一切**
    对局部路径很省事，但会把内存迁移、回收和系统整体稳定性成本转嫁到全局。
...
- **“地址没变，所以还是同一块东西。”**
  指针数值或 IOVA 没变，不代表背后的生命周期契约没有被释放、复用、COW 或映射回收打断。
...
### 7.5 用户页直通、零拷贝与长期 pin

RDMA、加速器、用户态 I/O 和零拷贝路径会把程序页直接暴露给设备或另一个 actor。此时最重要的问题不只是能不能访问，而是页能稳定多久、pin 的系统成本谁承担、IOMMU 与权限边界如何设置、完成之后谁负责把对象语义还给程序。

### 7.6 连续流设备和环形缓冲区

摄像头、音频、ADC 与其他连续流设备最关键的不是单次翻译，而是长期循环中的 ownership、wraparound、completion 粒度和消费粒度是否一致。
...
### 8.5 NVMe / xHCI / NIC：控制结构和 payload 生命周期不同

NVMe submission/completion queue、xHCI transfer/event ring、NIC RX/TX ring 都说明同一事实：队列和描述符常是长期共享控制结构，payload 页则更像按事务滚动使用的数据投影。它们都“在内存里”，但生命周期、同步责任和复用方式并不一样。
...
- 在代码和文档里写清 CPU -> device、device -> CPU 的 ownership 切换点
...
7. 为什么“地址有效”和“DMA 生命周期安全”不是同一个判断？
8. 哪四种生命周期最容易被误看成一条线？
9. 为什么 bounce buffer 不是纯粹失败退路，而是一种稳定化手段？
